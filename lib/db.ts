...
const dbPath = process.env.NODE_ENV === "production" ? "/tmp/appointments.db" : path.join(process.cwd(), "appointments.db");
export const db: Database.Database = useSupabase ? (null as any) : new Database(dbPath);

// ----- Helpers: run query with Supabase or SQLite -----
...
export async function insertService(row: { name: string; durationMinutes?: number; price?: number; category?: string }): Promise<any> {
  if (supabase) {
  ...
  const r = db.prepare("INSERT INTO services (name, durationMinutes, price, category) VALUES (?,?,?,?)").run(
    row.name, row.durationMinutes ?? 60, row.price ?? 0, row.category ?? "general"
  );
  return db.prepare("SELECT * FROM services WHERE id = ?").get(Number(r.lastInsertRowid)) as any;
}

export async function updateService(id: number, row: Partial<{ name: string; durationMinutes: number; price: number; category: string }>): Promise<any> {
  if (supabase) {
  ...
  db.prepare("UPDATE services SET name=?, durationMinutes=?, price=?, category=? WHERE id=?").run(
    updates.name, updates.durationMinutes, updates.price, updates.category, id
  );
...
export async function insertProfessional(name: string): Promise<any> {
  if (supabase) {
    const { data, error } = await supabase.from("professionals").insert({ name }).select("*").single();
    if (error) throw error;
    return data;
  }
...
export async function updateProfessional(id: number, name: string): Promise<any> {
  if (supabase) {
    const { data, error } = await supabase.from("professionals").update({ name }).eq("id", id).select("*").single();
    if (error) throw error;
    return data;
  }
...
export async function getProfessionalServices(professionalId: number): Promise<any[]> {
  if (supabase) {
    ...
    const { data: services } = await supabase.from("services").select("*").in("id", data.map((d) => d.service_id));
    return services ?? [];
  }
  return (db.prepare(
...
export async function setProfessionalServices(professionalId: number, serviceIds: number[]): Promise<void> {
  if (supabase) {
...
export async function getProfessionalSchedule(professionalId: number): Promise<any[]> {
  ...
  return (db.prepare("SELECT * FROM professional_schedule WHERE professional_id = ? ORDER BY day_of_week, start_time").all(professionalId) as any[]);
}

export async function setProfessionalSchedule(professionalId: number, schedule: { dayOfWeek: number; startTime: string; endTime: string }[]): Promise<void> {
  if (supabase) {
    await supabase.from("professional_schedule").delete().eq("professional_id", professionalId);
    if (schedule.length) {
      const rows = schedule
        .filter((s) => s.dayOfWeek != null && s.startTime != null && s.endTime != null)
        .map((s) => ({ professional_id: professionalId, day_of_week: s.dayOfWeek, start_time: s.startTime, end_time: s.endTime }));
      if (rows.length) {
  ...
  for (const s of schedule) {
    if (s.dayOfWeek != null && s.startTime != null && s.endTime != null) stmt.run(professionalId, s.dayOfWeek, s.startTime, s.endTime);
  }
}

// ----- Blocked slots -----
export async function getBlockedSlots(filters: { date?: string; professionalId?: number }): Promise<any[]> {
  if (supabase) {
    let q = supabase.from("blocked_slots").select("*").order("date").order("start_time");
    if (filters.date) q = q.eq("date", filters.date);
    if (filters.professionalId != null) q = q.or(`professional_id.is.null,professional_id.eq.${filters.professionalId}`);
  ...
  if (filters.professionalId != null) { query += " AND (professional_id IS NULL OR professional_id = ?)"; params.push(filters.professionalId); }
...
export async function insertBlockedSlot(row: {
  ...
  ).run(row.date, row.start_time, row.end_time, row.professional_id ?? null, row.reason ?? null);
...
// ----- Appointments -----
export async function getAppointments(filters: { date?: string; professionalId?: number; limit?: number }): Promise<any[]> {
  if (supabase) {
    let q = supabase
      .from("appointments")
  ...
  `;
  const params: (string | number)[] = [];
  if (filters.date) { query += " AND substr(a.dateTime, 1, 10) = ?"; params.push(filters.date); }
  if (filters.professionalId != null) { query += " AND a.professional_id = ?"; params.push(filters.professionalId); }
...
export async function insertAppointment(row: {
  ...
  db.prepare(
    "INSERT INTO appointments (clientName, phone, email, treatment, dateTime, durationMinutes, googleEventId, token, source, service_id, professional_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(
    payload.clientName, payload.phone, payload.email, payload.treatment, payload.dateTime, payload.durationMinutes,
    payload.googleEventId, payload.token, payload.source, payload.service_id, payload.professional_id
  );
...
export async function updateAppointmentDateTime(id: number, dateTime: string): Promise<any> {
  if (supabase) {
    const { data, error } = await supabase.from("appointments").update({ dateTime }).eq("id", id).select("*").single();
    if (error) throw error;
    return data;
  }
...
// For availability: get appointments by date prefix and status
export async function getBookedSlotsForDate(dateStr: string): Promise<{ dateTime: string; durationMinutes: number; professional_id: number | null }[]> {
  if (supabase) {
    const { data, error } = await supabase
      .from("appointments")
...
// Professional IDs that offer a service
export async function getProfessionalIdsForService(serviceId: number): Promise<number[]> {
  if (supabase) {
    const { data, error } = await supabase.from("professional_services").select("professional_id").eq("service_id", serviceId);
    if (error) throw error;
    return (data ?? []).map((r) => r.professional_id);
  }
  return (db.prepare("SELECT professional_id FROM professional_services WHERE service_id = ?").all(serviceId) as any[]).map((r) => r.professional_id);
}

// Schedule for a professional on a day
export async function getScheduleForProfessionalAndDay(professionalId: number, dayOfWeek: number): Promise<{ start_time: string; end_time: string }[]> {
  if (supabase) {
    const { data, error } = await supabase
      .from("professional_schedule")
  ...
  return (db.prepare("SELECT start_time, end_time FROM professional_schedule WHERE professional_id = ? AND day_of_week = ?").all(professionalId, dayOfWeek) as any[]);
}

// Blocked slots for a date and optional professional list
export async function getBlockedSlotsForDate(dateStr: string, professionalIds: number[]): Promise<{ start_time: string; end_time: string; professional_id: number | null }[]> {
  if (supabase) {
    const { data, error } = await supabase
      .from("blocked_slots")
  ...
  return (db.prepare(
    `SELECT start_time, end_time, professional_id FROM blocked_slots WHERE date = ? AND (professional_id IS NULL OR professional_id IN (${placeholders}))`
