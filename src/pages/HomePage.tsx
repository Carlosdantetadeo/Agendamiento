...
const COLORS = { bone: "#FDFBF9", graphite: "#2C2C2C", beige: "#F3EFEC", gold: "#C5A059" };

export default function HomePage() {
  const [services, setServices] = useState<Service[]>([]);
  const [professionals, setProfessionals] = useState<Professional[]>([]);
  const [step, setStep] = useState<"service" | "professional" | "date" | "slot" | "form">("service");
  ...
  const [lastBooking, setLastBooking] = useState<{ clientName: string; phone: string; serviceName: string; date: string; time: string; token?: string } | null>(null);
  ...
  const professionalsForService = service
    ? professionals.filter((p) => true)
    : [];
      ...
      <main className="max-w-md mx-auto px-6 pb-20">
          ...
          {status === "success" && lastBooking ? (
            ...
            <form onSubmit={handleBook} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest font-semibold text-[#2C2C2C]/50 ml-1">Servicio</label>
                <select
                  value={service?.id ?? ""}
                  onChange={(e) => {
              ...
              {/* Step: Professional (optional but we show it) */}
              {service && (
                <div className="space-y-2">
                  <label className="text-[10px] uppercase tracking-widest font-semibold text-[#2C2C2C]/50 ml-1">Profesional (opcional)</label>
                  <select
              ...
              {date && service && (
                <div className="space-y-4">
                  ...
                  {loadAvail ? (
                    <div className="flex items-center gap-2 text-[#2C2C2C]/60"><Loader2 className="w-4 h-4 animate-spin" /> Cargando...</div>
                  ) : (
                      ...
                      {availability.map((a) => (
                        <button
                          ...
                          className={`py-2 rounded-lg text-xs font-medium border transition-all ${
                            slot === a.time ? "bg-[#2C2C2C] text-white border-[#2C2C2C]" : "bg-white border-[#F3EFEC] hover:border-[#2C2C2C]/20"
                          }`}
                        >
                          {a.time}
                        </button>
                  ...
                  {!loadAvail && availability.length === 0 && date && service && (
                    <p className="text-sm text-amber-700">No hay horarios disponibles este día. Elige otra fecha o configura horarios en Admin.</p>
                  )}
                </div>
              )}
              ...
              {/* Client data */}
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest font-semibold text-[#2C2C2C]/50 ml-1">Nombre</label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest font-semibold text-[#2C2C2C]/50 ml-1">Teléfono (Perú)</label>
              ...
              <div className="space-y-2">
                <label className="text-[10px] uppercase tracking-widest font-semibold text-[#2C2C2C]/50 ml-1">Email (opcional)</label>
                <input
                  type="email"
              ...
              >
                <div className="relative z-10 flex items-center justify-center gap-2">
                  {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Confirmar Cita <ChevronRight className="w-4 h-4" /></>}
                </div>
              </button>
            </form>
          )}
        ...
        <div className="mt-12 text-center space-y-4">
          <div className="flex justify-center gap-8 text-[10px] uppercase tracking-[0.2em] text-[#2C2C2C]/40">
            <span className="flex items-center gap-1.5"><Sparkles className="w-3 h-3" /> Alta Gama</span>
            <span>•</span>
            <span>Cuidado Experto</span>
          </div>
          <p className="text-[11px] text-[#2C2C2C]/30">Lima, Perú — Cuidado de la piel profesional</p>
          <Link to="/admin" className="text-[11px] text-[#2C2C2C]/50 hover:text-[#C5A059]">Panel admin</Link>
        </div>
      </main>
    </div>
  );
...
function SuccessStep({ lastBooking }: { lastBooking: { clientName: string; serviceName: string; date: string; time: string; token?: string } }) {
    ...
    <div className="p-6 bg-emerald-50 border border-emerald-100 rounded-2xl space-y-4">
      ...
      {link && (
        <p className="text-xs text-[#2C2C2C]/60 break-all">
          Para cancelar o reprogramar: <a href={link} className="text-[#C5A059] underline">{link}</a>
        </p>
      )}
    </div>
  );
